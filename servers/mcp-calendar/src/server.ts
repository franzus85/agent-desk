...
async function handleLine(line: string): Promise<void> {
  ...
  try {
    ...
    process.stderr.write(`[mcp-calendar] Ignoring non-JSON line: ${line}\n`);
    return;
  }

  const parsed = jsonRpcRequestSchema.safeParse(raw);
  if (!parsed.success) {
    process.stderr.write(`[mcp-calendar] Ignoring malformed request: ${parsed.error.message}\n`);
    return;
  }
...
process.stderr.write("[mcp-calendar] server ready\n");
